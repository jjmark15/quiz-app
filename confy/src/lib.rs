...
//! use confy::MissingConfigFileAction::Create;
//!
...
//! let cfg = confy::load("my-app-name", Create)?;
//!     Ok(())
//! }
//! ```
//!
...
/// struct MyConfig {}
///
/// let cfg: MyConfig = confy::load("my-app-name", Create)?;
/// # Ok(())
/// # }
/// ```
pub fn load<T: Serialize + DeserializeOwned + Default>(
    name: &str,
    missing_config_action: MissingConfigFileAction,
) -> Result<T, ConfyError> {
    ...
    load_path(path, missing_config_action)
}
...
pub fn load_path<T: Serialize + DeserializeOwned + Default>(
    ...
    match File::open(&path) {
        ...
        Err(e) if e.kind() == NotFound => match missing_config_action {
            MissingConfigFileAction::Create => {
            ...
            MissingConfigFileAction::Nothing => Err(ConfyError::GeneralLoadError(e)),
        },
        Err(e) => Err(ConfyError::GeneralLoadError(e)),
    }
}

pub enum MissingConfigFileAction {
    Create,
    Nothing,
}
