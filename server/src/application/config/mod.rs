...
impl ConfigProfile {
    fn from_env<Reader: EnvReader>(env_reader: &Reader) -> Result<ConfigProfile, ConfigError> {
        ...
        match env_reader.read(key) {
            ...
            Err(_err) => {
                log::debug!("{}", log_string(&ConfigError::ApplicationProfileEnvNotSet));
                Ok(ConfigProfile::DEFAULT)
            }
        }
    }
...
pub(crate) mod tests {
    ...
    use crate::application::config::env::tests::mock_env_reader;
    use crate::application::config::env::MockEnvReader;
